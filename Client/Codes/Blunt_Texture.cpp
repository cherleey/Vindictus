...
CBlunt_Texture::CBlunt_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CBlunt_Texture::~CBlunt_Texture(void)
{

}

HRESULT CBlunt_Texture::Ready_GameObject(_vec3 vAngle)
{
...
_int CBlunt_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CBlunt_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CBlunt_Texture::Ready_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Component(SCENE_STAGE, L"Component_Texture_Blunt");
...
CBlunt_Texture* CBlunt_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CBlunt_Texture* pInstance = new CBlunt_Texture(pGraphicDev);

	if(FAILED(pInstance->Ready_GameObject(vAngle)))
	{
		MSG_BOX("CBlunt_Texture Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CBlunt_Texture::Free(void)
{	
...
void CBlunt_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CBlunt_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
...
void CBlunt_Texture::SetScale(_vec3 vScale)
{
	m_fCX = vScale.x;
	m_fCY = vScale.y;
	m_fCZ = vScale.z;
