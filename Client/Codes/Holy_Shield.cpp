...
CHoly_Shield_Texture::CHoly_Shield_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CHoly_Shield_Texture::~CHoly_Shield_Texture(void)
{

}

HRESULT CHoly_Shield_Texture::Ready_GameObject(_vec3 vAngle)
{
...
_int CHoly_Shield_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CHoly_Shield_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CHoly_Shield_Texture::Ready_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Component(SCENE_STAGE, L"Component_Texture_Holy_Shield");
...
CHoly_Shield_Texture* CHoly_Shield_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CHoly_Shield_Texture* pInstance = new CHoly_Shield_Texture(pGraphicDev);

	if(FAILED(pInstance->Ready_GameObject(vAngle)))
	{
		MSG_BOX("CHoly_Shield_Texture Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CHoly_Shield_Texture::Free(void)
{	
...
void CHoly_Shield_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CHoly_Shield_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
...
void CHoly_Shield_Texture::SetScale(_vec3 vScale)
{
	m_fCX = vScale.x;
	m_fCY = vScale.y;
	m_fCZ = vScale.z;
