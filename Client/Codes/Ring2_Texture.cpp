...
CRing2_Texture::CRing2_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CRing2_Texture::~CRing2_Texture(void)
{

}

HRESULT CRing2_Texture::Ready_GameObject(_vec3 vAngle)
{
...
_int CRing2_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CRing2_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CRing2_Texture::Ready_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Component(SCENE_STAGE, L"Component_Texture_Ring2");
...
CRing2_Texture* CRing2_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CRing2_Texture* pInstance = new CRing2_Texture(pGraphicDev);
...
void CRing2_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CRing2_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
...
void CRing2_Texture::SetScale(_vec3 vScale)
{
	m_fCX = vScale.x;
	m_fCY = vScale.y;
	m_fCZ = vScale.z;
