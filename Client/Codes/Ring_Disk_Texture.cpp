...
CRing_Disk_Texture::CRing_Disk_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CRing_Disk_Texture::~CRing_Disk_Texture(void)
{

}

HRESULT CRing_Disk_Texture::Ready_GameObject(_vec3 vAngle)
{
...
_int CRing_Disk_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CRing_Disk_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CRing_Disk_Texture::Ready_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Component(SCENE_STAGE, L"Component_Texture_Ring_Disk");
...
CRing_Disk_Texture* CRing_Disk_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CRing_Disk_Texture* pInstance = new CRing_Disk_Texture(pGraphicDev);
...
void CRing_Disk_Texture::Free(void)
{	
...
void CRing_Disk_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CRing_Disk_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
...
void CRing_Disk_Texture::SetScale(_vec3 vScale)
{
	m_fCX = vScale.x;
	m_fCY = vScale.y;
	m_fCZ = vScale.z;
