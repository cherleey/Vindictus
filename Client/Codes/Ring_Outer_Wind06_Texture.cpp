...
CRing_Outer_Wind06_Texture::CRing_Outer_Wind06_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CRing_Outer_Wind06_Texture::~CRing_Outer_Wind06_Texture(void)
{

}

HRESULT CRing_Outer_Wind06_Texture::Ready_GameObject(_vec3 vAngle)
{
...
_int CRing_Outer_Wind06_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CRing_Outer_Wind06_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CRing_Outer_Wind06_Texture::Ready_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Component(SCENE_STAGE, L"Component_Texture_Ring_Outer_Wind06");
...
CRing_Outer_Wind06_Texture* CRing_Outer_Wind06_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CRing_Outer_Wind06_Texture* pInstance = new CRing_Outer_Wind06_Texture(pGraphicDev);
...
void CRing_Outer_Wind06_Texture::Free(void)
{	
...
void CRing_Outer_Wind06_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CRing_Outer_Wind06_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
...
void CRing_Outer_Wind06_Texture::SetScale(_vec3 vScale)
{
	m_fCX = vScale.x;
	m_fCY = vScale.y;
	m_fCZ = vScale.z;
