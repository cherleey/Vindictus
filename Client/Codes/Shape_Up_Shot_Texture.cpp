...
CShape_Up_Shot_Texture::CShape_Up_Shot_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CShape_Up_Shot_Texture::~CShape_Up_Shot_Texture(void)
{

}

HRESULT CShape_Up_Shot_Texture::Ready_GameObject(_vec3 vAngle)
{
...
_int CShape_Up_Shot_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CShape_Up_Shot_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CShape_Up_Shot_Texture::Ready_Component(void)
{
...
CShape_Up_Shot_Texture* CShape_Up_Shot_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CShape_Up_Shot_Texture* pInstance = new CShape_Up_Shot_Texture(pGraphicDev);

	if(FAILED(pInstance->Ready_GameObject(vAngle)))
	{
		MSG_BOX("CShape_Up_Shot_Texture Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CShape_Up_Shot_Texture::Free(void)
{	
...
void CShape_Up_Shot_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CShape_Up_Shot_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
...
void CShape_Up_Shot_Texture::SetScale(_vec3 vScale)
{
	m_fCX = vScale.x;
	m_fCY = vScale.y;
	m_fCZ = vScale.z;
