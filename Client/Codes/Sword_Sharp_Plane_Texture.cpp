...
CSword_Sharp_Plane_Texture::CSword_Sharp_Plane_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CSword_Sharp_Plane_Texture::~CSword_Sharp_Plane_Texture(void)
{

}

HRESULT CSword_Sharp_Plane_Texture::Ready_GameObject(_vec3 vAngle)
{
	if(FAILED(Ready_Component()))
		return E_FAIL;

	m_pTransformCom->Set_Infomation(Engine::CTransform::INFO_SCALE, 70.f, 70.f, 70.f);
...
_int CSword_Sharp_Plane_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CSword_Sharp_Plane_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CSword_Sharp_Plane_Texture::Ready_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Component(SCENE_STAGE, L"Component_Texture_Sword_Sharp_Plane");
...
CSword_Sharp_Plane_Texture* CSword_Sharp_Plane_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CSword_Sharp_Plane_Texture* pInstance = new CSword_Sharp_Plane_Texture(pGraphicDev);

	if(FAILED(pInstance->Ready_GameObject(vAngle)))
	{
		MSG_BOX("CSword_Sharp_Plane_Texture Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CSword_Sharp_Plane_Texture::Free(void)
{	
...
void CSword_Sharp_Plane_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CSword_Sharp_Plane_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
