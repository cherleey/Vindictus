...
CYellow_Blunt_Texture::CYellow_Blunt_Texture(LPDIRECT3DDEVICE9 pGraphicDev)
...
CYellow_Blunt_Texture::~CYellow_Blunt_Texture(void)
{

}

HRESULT CYellow_Blunt_Texture::Ready_GameObject(_vec3 vAngle)
{
...
_int CYellow_Blunt_Texture::Update_GameObject(const _float& fTimeDelta)
{	
...
void CYellow_Blunt_Texture::Render_GameObject(void)
{
	if(!m_bRender)
	{
		m_bStart = false;
...
HRESULT CYellow_Blunt_Texture::Ready_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Component(SCENE_STAGE, L"Component_Texture_Yellow_Blunt");
...
CYellow_Blunt_Texture* CYellow_Blunt_Texture::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vAngle)
{
	CYellow_Blunt_Texture* pInstance = new CYellow_Blunt_Texture(pGraphicDev);

	if(FAILED(pInstance->Ready_GameObject(vAngle)))
	{
		MSG_BOX("CYellow_Blunt_Texture Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CYellow_Blunt_Texture::Free(void)
{	
...
void CYellow_Blunt_Texture::Rotate(_vec3 vAngle)
{
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_X, D3DXToRadian(vAngle.x));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Y, D3DXToRadian(vAngle.y));
	m_pTransformCom->Rotation(Engine::CTransform::ANGLE_Z, D3DXToRadian(vAngle.z));
...
void CYellow_Blunt_Texture::SetPosition(_vec3 vPos)
{
	m_fX = vPos.x;
	m_fY = vPos.y;
	m_fZ = vPos.z;
...
void CYellow_Blunt_Texture::SetScale(_vec3 vScale)
{
	m_fCX = vScale.x;
	m_fCY = vScale.y;
	m_fCZ = vScale.z;
