                     ...
                     int       nCmdShow)
{
	...
	if(FAILED(Engine::Ready_Timers(L"Timer_Default")))
		return FALSE;

	if(FAILED(Engine::Ready_Timers(L"Timer_60")))
		return FALSE;

	if(FAILED(Engine::Ready_Frames(L"Frame:60", 100.0f)))
		return FALSE;
	...
	while(true)
	{
		...
		Engine::SetUp_TimeDelta(L"Timer_Default");

		if(true == Engine::Permit_Call(L"Frame:60", Engine::Get_TimeDelta(L"Timer_Default")))
		{
			Engine::SetUp_TimeDelta(L"Timer_60");

			if(pMainApp->Update_MainApp(Engine::Get_TimeDelta(L"Timer_60")) & 0x80000000)
				break;
	...
	if(0 != ::Safe_Release(pMainApp))
		MessageBox(NULL, L"CMainApp Release Failed", L"System Error", MB_OK);
